...
void ArmorDetector::loopStart(Shooter &shooter,VideoWriter &out)
{
    ...
    //    }
    //stop_flag_ = false;
    cv::VideoCapture cap("/home/allspark002/build-bubing5-3-unknown-Debug/Tue Jun_19_21:26:33_2018.avi");
...
int armor::ArmorDetector::findTargets(cv::Mat &src, cv::Mat &mask,cv::Mat &show) {
    ...
    if(contours0.size()>=2)
    {
        ...
        for (uint i=0; i < contours0.size(); i++)
        {
            ...
            int y_i=rect_i.center.y;
            if(abs(y_k-y_i)<=h_one)      // 两个轮廓基本处于水平
...
//    }
    ...
    if(contours1.size()>=2)
    {
        if(contours1.size()==2)
        {
            ...
            int contours_0_size = contours1[0].size();
            int contours_1_size = contours1[1].size();
//            std::cout << "contours_0_size:" << contours_0_size << std::endl;
//            std::cout << "contours_1_size:" << contours_1_size << std::endl;
            contours_target.resize(contours_0_size + contours_1_size);

            for (size_t m = 0; m < contours_0_size; m++)
                contours_target[m] = contours1[0][m];
            for (size_t n = 0; n < contours_1_size; n++)
                contours_target[n + contours_0_size] = contours1[1][n];

            area_a = contourArea(contours_target);
            std::cout << "two armor area: " << "\t" << area_a << std::endl;

            if (area_a < 15)    //八字型轮廓的面积
            {
                for (uint i=0; i < contours1.size(); i++)
                {
        ...
        if(contours1.size()==3)
        {
            ...
            for (uint i = 0; i < 3; i++)
            {
                ...
                //if(contourArea(contours1[i],false)>50)
                        cout<<"                       "<<rect_i.size.height<<"distance   stick"<<contourArea(contours1[i],false)<<"Area "<<endl;
                //area_[i] = contourArea(contours1[i]);
...
//            double area_smallest = 0, area_biggest = 0, area_mid, small_k, big_k;
//            //area_biggest = std::max(std::max(area_[0],area_[1]), area_[2]);
//            //area_smallest = std::min(std::min(area_[0],area_[1]), area_[2]);
//            for (uint i = 0; i < 3; i++)
//            {
//                if (area_smallest > area_[i])
//                {
//                    area_smallest = area_[i];
//                    small_k = i;
//                }
//                if (area_biggest < area_[i])
//                {
//                    area_biggest = area_[i];
//                    big_k = i;
//                }
//            }
//            if (area_[0]>area_smallest && area_[0] < area_biggest)
//            {
//                area_mid = area_[0];
//                contours2.push_back(contours1[0]);
//            }
//            else if (area_[1]>area_smallest && area_[1] < area_biggest)
//            {
//                area_mid = area_[1];
//                contours2.push_back(contours1[1]);
//            }
//            else
//            {
//                area_mid = area_[2];
//                contours2.push_back(contours1[2]);
//            }

//            if ((area_biggest - area_mid) > (area_mid - area_smallest))
//                contours2.push_back(contours1[small_k]);
//            else
//                contours2.push_back(contours1[big_k]);
...
//                std::cout << "x_i "<<  x_i[i]  << std::endl;
            ...
            for (uint i = 0; i < 4; i++)
            {
                ...
                if((x_!=min_i)&&(x_!=max_i))
                    contours2.push_back(contours1[i]);
//                if(rect_.size.height>10)
//                       contours2.push_back(contours1[i]);
    ...
    if(contours2.size()>=2)
    {
        ...
        for (size_t i = 0; i < contours2.size(); i++)
        {
            ...
            for (size_t j = i + 1; j < contours2.size(); j++)
            {
                ...
                std::cout << "box_left_angle:" << box_left_angle <<"\n" << "box_right_angle:" << box_right_angle<<std::endl;
#if filter3
           ...
           std::cout << " delta_left_right_angle "<< delta_left_right_angle<< " <15"<<std::endl;
            std::cout << " rect_i.angle "<<rect_i.angle << " rect_j.angle "<<rect_j.angle<<std::endl;
                ...
                if (
                        ...
                        &&height_i / height_j<2.2
                        &&height_i / height_j>0.4
                        ...
                        && (delta_left_right_angle <7)//LITTLE15 BIG10
                        //&& (delta_left_right_angle >)
                        &&((rect_i.angle+45)*(rect_j.angle+45)>=0||rect_i.angle==-90||rect_j.angle==-90)
                        &&limit==false
...
float y_correction()
{
   ...
   distance = 3.600*0.001*0.06*77.500*0.588 / (D_height*0.300*0.001);

    if (distance > 4)
    {
        correct_angle = -6;//1.35;
    }
...
int armor::Shooter::angle_analysis(cv::Point &Target_point,cv::Point2f &convert_angle,double &Distance)
{
    ...
    distance = 3.600*0.001*0.06*77.500*0.588 / (D_height*0.300*0.001);
     //tmp_direction.y += y_correction();
        cout<<"qqqqqqqqqqqqqqqqqqqqq"<<distance<<endl;
        cout << ">>>>>>>>>>>>x_angle :" << tmp_direction.x << endl;
        cout << ">>>>>>>>>>>>y_angle :" << tmp_direction.y << endl;
    convert_angle.x=tmp_direction.x-1;

    convert_angle.y=tmp_direction.y-1;
