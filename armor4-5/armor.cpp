...
void ArmorDetector::loopStart(V4L_capture &v4l_capture, Shooter &shooter, Serial_commu &serial_commu)
{
    ...
    do
    {
        ...
        if(get_one==1)
        {
            ...
            if(sendData[2]>500)
                sendData[2]=1000;
            serial_commu.serial_sendarmor(usart_0, sendData, 3);
    ...
    if(contours2.size()>=2)
    {
        ...
        for (size_t i = 0; i < contours2.size(); i++)
        {
            ...
            for (size_t j = i + 1; j < contours2.size(); j++)
            {
                ...
                if (
                        abs(box_up_angle) <max_up_down_angle
                        ...
                        &&averageHeight*5>distance_center
