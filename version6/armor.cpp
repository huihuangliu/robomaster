...
void ArmorDetector::loopStart(Shooter &shooter)
{
    //    if (!stop_flag_) {
...
int armor::ArmorDetector::findTargets(cv::Mat &src, cv::Mat &mask,cv::Mat &show) {
    ...
    for (size_t i = 0; i < contours.size(); i++)
    {
        float d[4];
        float height, width;
        cv::RotatedRect rect = cv::minAreaRect(contours[i]);     //最小面积包围矩形框
        cv::Rect rect_bound = cv::boundingRect(contours[i]);    //最小包围矩形框
        cv::Point2f vertex[4];
        rect.points(vertex);
        ...
        if (rect_bound.height>rect_bound.width)
        {
            for (int i = 0; i<4; i++)
            {
                d[i] = dist(vertex[i].x, vertex[i].y, vertex[(i + 1) % 4].x, vertex[(i + 1) % 4].y);   //计算四个顶点的距离
            }

            float angle_light = angle(vertex[0].x, vertex[0].y, vertex[1].x, vertex[1].y);    //矩形框的水平角度
            if (d[1]>d[0])   //确定矩形框的高和宽
            {
                height = d[1];
                width = d[0];
            }
            ...
            if (angle_light<min_angle     //角度条件判断
                    && contours[i].size() > min_contours_size
                    &&height>min_contour_height)
            {
                contours0.push_back(contours[i]);      //第一次筛选的结果contours0
            }
        }
    }
    ...
    if(contours0.size()>=2)
    {
        ...
        for (uint i = 0; i < contours0.size(); i++)
        {
            ...
            for (uint j = i+1; j < contours0.size(); j++)
            {
                ...
                if(abs(y_i-y_j)<=h)    //比较两两轮廓最小包围矩形框中心y坐标的差，如果是为了判断两个轮廓的水平高度，
                    y[i]+=1;            //为什么是小于等于h？h作为余量也太大了吧
            }
        }

        int max_y=0;
        for (uint i=0;i<contours0.size();i++)    //找出处于同一水平位置的最大轮廓数量
            if(max_y<y[i])
            {
                max_y=y[i];
                k=i;
        ...
        for (uint i=0; i < contours0.size(); i++)
        {
            ...
            int y_i=rect_i.center.y;
            if(abs(y_k-y_i)<=h)
                contours1.push_back(contours0[i]);   //把处于同一水平位置的轮廓存放在contours1，上述过程有些繁琐
        }
    }
...
//    }
    ...
    if(contours1.size()>=2)
    {
        if(contours1.size()==2)   //轮廓数目为2
        {
            for (uint i=0; i < contours1.size(); i++)
            {
                contours2.push_back(contours1[i]);    //符合情况则存放在contours2
            }
        }
        if(contours1.size()==3)    //轮廓数目为3
        {
            int x_i[3],x_mid;
            for (uint i = 0; i < 3; i++)
            {
                RotatedRect rect_i = minAreaRect(contours1[i]);
                x_i[i]=rect_i.center.x;     //每个轮廓的中心x坐标
            }
            ...
            for (uint i = 0; i < 3; i++)
            {
                ...
                {
                    x_mid=x_i[i];     //中间的那个轮廓的中心x坐标x_mid
                    contours2.push_back(contours1[i]);    //中间的那个轮廓存放在contours2
                }
            }
            int small_i=0;
            int big_i=0;
            for (uint i = 0; i < 3; i++)    //确定最左边轮廓和最右边轮廓
            {
                if(x_i[i]!=x_mid)
                {
                    if(x_i[i]<x_mid)
            ...
            if(abs(x_i[small_i]-x_mid)<abs(x_i[big_i]-average_x))   //选择距离最近的两个轮廓，存放在contours2
                contours2.push_back(contours1[small_i]);
            else
                contours2.push_back(contours1[big_i]);
        }

        if(contours1.size()==4)     //轮廓数目为4
        {
            int x_i[4];
            for (uint i = 0; i < 4; i++)
            {
            ...
            int min_i=min(min(x_i[0],x_i[1]),min(x_i[2],x_i[3]));    //中心x坐标最小的轮廓
            int max_i=max(max(x_i[0],x_i[1]),max(x_i[2],x_i[3]));    //中心x坐标最大的轮廓
            for (uint i = 0; i < 4; i++)
            {
                if((x_i[i]!=min_i)&&(x_i[i]!=max_i))     
                    contours2.push_back(contours1[i]);      //把中间的两个轮廓存放在contours2
            }
        }
        int height_5;
        if(contours1.size()>=5)    //轮廓数目大于等于5
        {
            ...
            for (uint i = 0; i < contours1.size(); i++)
            {
                ...
                x_i[i]=rect_i.center.x;    //每个轮廓中心x坐标
                ave+=x_i[i];
            }
            ave=ave/contours1.size();     //所有轮廓的中心x坐标
            height_5=height_5/contours1.size();    //所有轮廓的平均高度

            for (uint i = 0; i < contours1.size(); i++)
            {
                if(abs(x_i[i]-ave)<height_5*2/3)      //轮廓与所有轮廓的中心x坐标的距离小于平均高度的2/3倍
    ...
    if(contours2.size()>=2)
    {
        ...
        for (size_t i = 0; i < contours2.size(); i++)
        {
            ...
            for (size_t j = i + 1; j < contours2.size(); j++)
            {
                ...
                for (size_t m = 0; m < 4; m++)    //第一个轮廓
                {
                    if (up_point_i.y > vertice_i[m].y)
                        up_point_i = vertice_i[m];     //找到矩形框的上顶点
                    if (down_point_i.y < vertice_i[m].y)
                        down_point_i = vertice_i[m];    //找到矩形框的下顶点
                }
                for (int n = 0; n < 4; n++)      //第一个轮廓
                {
                ...
                double height_i = std::max(rect_i.size.height, rect_i.size.width);    //第一个轮廓的高
                double height_j = std::max(rect_j.size.height, rect_j.size.width);    //第二个轮廓的高
                double distance_center = dist(rect_i.center.x, rect_i.center.y, rect_j.center.x, rect_j.center.y);  //中心距
                double averageHeight = (height_i + height_j) / 2;

                if ((up_point_i.x - up_point_j.x) == 0)
                    box_up_angle = 0;
                else
                    box_up_angle = angle(up_point_i.x, up_point_i.y, up_point_j.x, up_point_j.y);   //两轮廓上边的角度

                if ((down_point_i.x - down_point_j.x) == 0)
                    box_down_angle = 0;
                else
                    box_up_angle = angle(down_point_i.x, down_point_i.y, down_point_j.x, down_point_j.y);    //两轮廓下边的角度

                if (vertice_i[0].x - vertice_i[1].x == 0)
                    box_left_angle = 0;
                else
                    box_left_angle = angle(vertice_i[0].x, vertice_i[0].y, vertice_i[1].x, vertice_i[1].y); //两轮廓左侧的角度

                if (vertice_j[0].x - vertice_j[1].x == 0)
                    box_right_angle = 0;
                else
                    box_right_angle = angle(vertice_j[0].x, vertice_j[0].y, vertice_j[1].x, vertice_j[1].y);  //两轮廓右侧的角度
                ...
                if (
                        abs(box_up_angle) <max_up_down_angle      //上边角度  
                        &&abs(box_down_angle) <max_up_down_angle  
                        &&height_i / height_j<2                  //两灯条高度比  
                        &&height_i / height_j>0.5                 
                        &&abs(up_point_i.x-up_point_j.x)>averageHeight    //两轮廓上顶点x坐标之差
                        &&abs(abs(up_point_i.x-up_point_j.x)-abs(down_point_i.x-down_point_j.x))<height_i
                        &&distance_center>averageHeight*2    //中心距
                        &&averageHeight*5>distance_center   //上下左右对称角度
                        && delta_up_down_angle <min_angle
                        && (delta_left_right_angle <4)
                        ...
                        )
                {
                    ...
                    int contours_i_size = contours2[i].size();   //第i个轮廓上像素点的个数
                    ...
                    for (size_t n = 0; n < contours_j_size; n++)
                        contours_temp_target[n + contours_i_size] = contours2[j][n]; //把筛选出的轮廓所有像素点存到一起
                    contours3.push_back(contours_temp_target);       //保存一对符合条件的轮廓的所有像素点在contours3
																	//contours3里每个元素是一对轮廓合在一起的像素点
                }
            }
        }
    }
    ...
    if(contours3.size()>=1)    //
    {
        ...
        {
            if(lost_frame<const_lost_frame)   //丢帧数小于设定值时
            {
                rect3 = boundingRect(contours3[i])+up_left;   //得到轮廓的最小包围矩形框
            }
            else
            {
                rect3 = boundingRect(contours3[i]);
            }
            if (makeRectSafe(rect3, src.size()) == false)   //判断是否超出边界
            {
                rect3 = cv::Rect(0, 0, src.cols, src.rows);    //如果超出边界则矩形框为整张图像
            }
            else
            {
                src(rect3).copyTo(copy_image);    //将最小包围矩形框的ROI区域截取赋给copy_image
            }

            cv::cvtColor(copy_image,gray_image,cv::COLOR_RGB2GRAY,0);
            cv::threshold(gray_image,threshold_image, 100, 255,cv::THRESH_BINARY);
            standard_template.create(gray_image.rows,gray_image.cols,gray_image.type());//产生标准模板

            makeTemplate(standard_template.cols,standard_template.rows,standard_template);  //产生模板
            //            imshow("standard_template",standard_template);
            //            imshow("threshold",threshold_image);
            double a=brightSums(standard_template);   //对标准模板亮点计数
            double b=brightSums(threshold_image);    //对二值图像的亮点计数

            double remind=b/a;     //计算相似度

            ///相似度筛选
            if(remind>0.3&&remind<4)
                if(threshold_image.cols>threshold_image.rows)
                {
                    contours4.push_back(contours3[i]);    //相似度满足的存放在contours4
                }
        }
    }
    std::cout<<"contours4="<<contours4.size()<<std::endl;
    if(contours4.size()>=1&&contours4.size()<3)     //装甲板个数为1个或2个时
    {
...
//        }
        ...
        for (size_t i = 0; i < angle_i.size(); i++)
        {
            if(angle_i[i]<smallest_angle)
            {
                ...
                contours_result=contours4[i];   //最终筛选出的灯条轮廓存在contours_result
                getTarget = 1;                 //得到最终目标装甲板
            }
        }
    ...
    if (getTarget == 1)
    {
        ...
        cv::RotatedRect rect_result = cv::minAreaRect(contours_result);  //两个灯条的最小矩形包围框
        ...
        if(dis[2]<min_dist)
            min_dist=dis[2];
        //最小距离
        D_height=min_dist;     //灯条高度
        //全局搜索
        if (lost_frame >= const_lost_frame)    //丢帧数量超出设定值
        {
            _res_last = cv::RotatedRect();

            for (size_t i = 0; i<4; i++)
                _res_last_point.push_back(vertex_result[i]);    
            _res_last = minAreaRect(_res_last_point);    //把之前的矩形框赋给_res_last

            for (size_t i = 0; i<4; i++)    //画出装甲板检测的结果
            {
        ...
        else
        {
            ...
            _res_last = minAreaRect(_res_last_point);      //最小矩形框
        ...
        if(bounding_rect.width<400)    //装甲板宽度小于400
        {
...
int armor::ArmorDetector::target_track(cv::Mat &src,cv::Mat &show)
{
    ...
    Is_recognizing = continue_shot;     //没有识别
    ...
    if (Is_recognizing == 1)     //识别到了目标
    {
        TargetGet_times++;
        TargetLost_times = 0;

        if (lost_frame >= const_lost_frame)    //丢帧大于设置值
        {
            for (int i = 0; i<4; i++)
        ...
        if (makeRectSafe(track_window, new_srcImage.size()) == false)      //如果超出边界，则全局搜索
        {
    ...
    if (track_object && continue_shot == 0)
    {
        ...
        if(standard_template.cols<100||((standard_template.cols>100)
                ...
                &&abs(D_height-standard_template.cols)<20
                )
        {
            double minVal;
            double maxVal;
            ...
            matchTemplate(image_roix, track_Image, result, CV_TM_SQDIFF_NORMED);    //模板匹配
            ...
            ///防漂移，参数可调
            if((abs(last_point.x-target_pixel.x)<march_target.width / 5)
                    &&target_pixel.x>march_target.width
                    &&(new_srcImage.cols-target_pixel.x)>march_target.width
                    &&march_target.width<march_target.height*4
