...
int armor::Serial_commu::send_data(int fd_, const int16_t *buffer)
{
    //printf("\tsend:%d\n", *buffer);
    return write(fd_, (char*)buffer, sizeof(uint16_t));
}
int armor::Serial_commu::send_dataarmor(int fd_, const int *buffer)
{
    //printf("\tsend:%d\n", *buffer);
    return write(fd_, (char*)buffer, sizeof(uint16_t));
}

int armor::Serial_commu::serial_send(int fd_, double *float_data,int len) // If recognized, add 1 to data_check; Give out 4 data in total: ff, x, y, (x+y+255+recognize).
{
    std::vector<int16_t> data;
    int16_t data_check;
    std::cout<<"serial_send"<<len<<std::endl;
    ...
    std::cout<<"bbbbb0"<<data[0]<<std::endl;
    std::cout<<"bbbbb1"<<data[1]<<std::endl;
    std::cout<<"bbbbb2"<<data[2]<<std::endl;
    send_data(fd_, &data_check);

    return 0;
}
int armor::Serial_commu::serial_sendarmor(int fd_, double *float_data,int len) // If recognized, add 1 to data_check; Give out 4 data in total: ff, x, y, (x+y+255+recognize).
{
    std::vector<int> data;
    int data_check=0;
    std::cout<<"serial_send"<<len<<std::endl;
...
void armor::Serial_commu::usart_init()
{
...
int armor::Serial_commu::set_opt(struct termios newtio,int fd,int nSpeed, int nBits, char nEvent, int nStop)
{
    /*步骤一，设置字符大小*/
